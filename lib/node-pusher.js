module.exports = (function() {
  ...
  Pusher.prototype.stats = function(channel, callback){
    var timestamp = parseInt(new Date().getTime() / 1000);

    var path = '/apps/' + this.options.appId + '/channels/' + channel + '/stats';
    ...
    var signData = ['GET', path, queryString].join('\n');
    var signature = crypto.createHmac('sha256', this.options.secret).update(signData).digest('hex');

    path = "http://" + this.domain + path + '?' + queryString + '&auth_signature=' + signature;
